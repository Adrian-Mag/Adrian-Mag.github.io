...
In a probabilistic setting, this relation is an observation kernel
\[
...
In a set-valued setting, the analogous object is a set-valued observation relation
\[
...
In the probabilistic setting, this model-data relation is encoded by an observation kernel
\[
...
\begin{intuitionbox}
The observation kernel is a machine that takes a model as input and outputs a probability law on
...
shifted-noise cloud is centred on what happened. The observation kernel describes what could
happen under each possible model.

\begin{checkpointbox}
The shifted-noise measure is observation-centred. The observation kernel is model-centred, or
generative.
\end{checkpointbox}
...
\begin{lessonbox}
SOLA pushes the observation kernel from data space to property space. It turns a model-data
...
Together with the observation kernel
\[
  L_{\mathrm{prob}}(m,\cdot),
\]
...
In the probabilistic case, this is an observation kernel
\[
  m\mapsto L_{\mathrm{prob}}(m,\cdot).
\]
