...
Second, in Section~\ref{sec:measure}, we solve the same problem again using the language of measure theory. The prior becomes a probability measure on the model space. The reporting mechanism becomes a likelihood kernel. The combined experiment becomes a joint measure. The Bayesian update becomes a posterior kernel.
...
The prior distribution becomes a probability measure on the model space. The reporter rule becomes a likelihood kernel from the model space to the data space. The joint table becomes a joint measure on \(\M\times\D\). The column sums become marginals. The normalized columns become a posterior kernel.
...
report probabilities \(\mathbb{P}(d\mid m)\) & likelihood kernel \(L(m,B)\) \\
...
\subsection{The likelihood kernel}
...
This object is called the likelihood kernel.

For each fixed hidden state \(m\in\M\), the map
\[
    B\longmapsto L(m,B)
\]
is a probability measure on \((\D,\SigmaD)\). In other words, once \(m\) is fixed, the likelihood kernel tells us the probability of every possible data event.
...
The likelihood kernel is just the reporter table from Section~\ref{sec:elementary}, rewritten as a kernel.
...
The prior measure \(\muMzero\) describes uncertainty about the hidden model. The likelihood kernel \(L\) describes how each hidden model produces reports.
...
The reporter rule became a likelihood kernel
\[
    L(m,B),
    \qquad
    B\in\SigmaD,
...
Together, the prior measure and likelihood kernel defined the joint measure
\[
    J(A\times B)
    =
    \int_A L(m,B)\,\muMzero(dm).
