...
\]
This family is the observation kernel. It tells us how plausible the observed datum would be under each candidate truth, but by itself it does not assign probabilities to the candidate truths.
...
Thus, the known-truth construction motivates the observation kernel:
\[
    \mathbf d
    \longmapsto
    \mu_{\mathbf d}.
...
% ============================================================
\section{The Observation Kernel}
% ============================================================
...
Thus, for each possible candidate truth \(\mathbf d\), we obtain a corresponding probability law for the observed datum. The role of the observation kernel is to collect all of these candidate-dependent observation laws into one object.
...
\subsection{The observation kernel as a map}
...
\[
    ...
    \mu_{\mathbf d}
\]
is the observation kernel. More explicitly, we may write
\[
...
Thus, the observation kernel has the direction
\[
    \text{candidate truth}
    \longmapsto
...
This direction is important. The observation kernel does not yet map observations to probability measures on truths. It maps possible truths to probability measures on observations.
...
\[
    ...
    p_N(\dobs-\mathbf d).
\]
The likelihood function is obtained by evaluating the observation kernel at the observed datum. It is a function of the candidate truth \(\mathbf d\), but it is not by itself a probability density over \(\mathbf d\).

\subsection{What the observation kernel can say}

The observation kernel can answer candidate-wise questions. For a proposed true datum \(\mathbf d\), it can ask:
...
This is the basic frequentist compatibility idea that will later become a confidence set. The observation kernel gives the sampling law under each candidate truth, and from those sampling laws one can decide which candidate truths would make the observation look ordinary, unusual, or extreme.
...
Thus, the observation kernel can compare candidate truths by asking how well each one explains the observed datum through the noise model.

\subsection{What the observation kernel cannot say alone}

The observation kernel does not, by itself, assign probabilities to candidate true data values. It tells us
\[
...
The observation kernel has the direction
\[
    \mathbf d
    \longmapsto
    \mu_{\mathbf d}.
...
\]
Such a statement requires a probability model for \(\dtrue\). The observation kernel does not provide one. It provides only the family of observation laws
\[
    \{\mu_{\mathbf d}:\mathbf d\in\Dspace\}.
\]

Therefore, the observation kernel is a crucial object shared by both Bayesian and frequentist reasoning, but it is not yet a full inference about the truth. It says what the observation model would look like under each candidate truth. What one does with that family of laws is where the Bayesian and frequentist constructions begin to diverge.
...
The observation kernel described in the previous section gives, for each candidate true datum \(\mathbf d\), a probability law for possible observations. It has the direction
\[
    \mathbf d
    \longmapsto
    \mu_{\mathbf d}.
...
Using the observation kernel, this can be written as
\[
    \mu_{\mathrm{pred}}(A)
    =
    \int_{\Dspace}
...
\subsection{The observation kernel inside the Bayesian construction}

The observation kernel appears inside the Bayesian construction as the conditional law of the observation given the epistemological true datum.
...
Therefore, the observation kernel is not uniquely Bayesian. It is the sampling part of the model. What is distinctively Bayesian is the multiplication of this sampling model by a prior measure on possible true data values.

\subsection{The posterior kernel}

The posterior kernel reverses the direction of the observation kernel.

The observation kernel has direction
\[
    \text{candidate truth}
    \longmapsto
...
\begin{description}[leftmargin=2.8cm,style=nextline]
    ...
    The observation kernel describes possible observations for each candidate truth.
...
\[
    ...
    \,\prior(d\mathbf d).
\]
Again, the prior is essential. It supplies the marginal law of the epistemological true datum and allows the observation kernel to be assembled into a joint probability measure.
...
Therefore, the observation kernel survives:
\[
    \mathsf L(\mathbf d,A)
    =
    \mu_{\mathbf d}(A).
...
That latter statement belongs to a Bayesian construction, or to another framework that introduces a post-data probability measure on possible true values. It is not supplied by the observation kernel alone.
...
This is the same candidate-wise logic already present in the observation kernel. The difference is that the frequentist uses this logic to construct tests, confidence sets, and decision procedures, rather than to construct a posterior measure over possible truths.
...
\subsection{Starting from the observation kernel}
...
The more general starting point is therefore the observation kernel itself. For each candidate true datum
\[
    \mathbf d\in\Dspace,
\]
...
\[
    ...
    H(\mathbf d,\mathbf W),
\]
where \(\mathbf W\) is a random variable representing all unresolved randomness in the measurement process. Then the observation kernel is
\[
    \mathsf L(\mathbf d,A)
    =
...
The important point is that the observation kernel already contains everything needed to describe the candidate-dependent sampling law. The notation
\[
    \mathsf L(\mathbf d,\cdot)
\]
...
\[
    ...
    \right\},
\]
where \(A_{\alpha}(\mathbf d)\) is chosen directly from the observation kernel.
...
The coverage statement now follows directly from the observation kernel.
...
The additive shifted-noise proof was only one special version of this argument. The general proof needs only the observation kernel and the candidate-dependent acceptance regions.
...
In terms of the observation kernel, the guarantee is
\[
    \sup_{\mathbf d\in\mathcal H_{\mathrm{danger}}}
    \mathsf L
    \left(
...
This is the general frequentist answer in safety-critical settings. Whether the observation law is additive, heteroscedastic, multiplicative, or nonlinear, the frequentist can work directly with the observation kernel and design rules with uniform error guarantees.
